    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Manage and track your outreach performance.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredCampaigns.length === 0 ? (
          <div className="col-span-full glass p-20 rounded-[48px] border-dashed border-border/60 text-center space-y-6">
            <div className="w-24 h-24 bg-muted rounded-full flex items-center justify-center mx-auto text-slate-300">
              <Mail size={48} />
            </div>
            <div className="max-w-xs mx-auto">
              <h3 className="text-xl font-bold text-slate-800">No campaigns found</h3>
              <p className="text-slate-500 font-medium mt-2">Try adjusting your search or create a new campaign to get started.</p>
            </div>
            <Link to="/campaigns/new">
          ...
          filteredCampaigns.map((campaign, i) => (
            <motion.div
              ...
              className="glass p-8 rounded-[40px] border-border/40 hover:border-primary/20 transition-all group relative overflow-hidden flex flex-col"
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <h3 className="text-xl font-bold text-slate-800 group-hover:text-primary transition-colors mb-2 truncate">{campaign.name}</h3>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8 mt-auto">
                  ...
                  <div className="bg-muted/50 p-4 rounded-2xl">
                    ...
                    <div className="text-lg font-bold text-slate-800">{new Date(campaign.createdAt).toLocaleDateString()}</div>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  <Link to={`/campaigns/${campaign.id}`} className="flex-1">
                    <button className="w-full bg-slate-800 hover:bg-slate-900 text-white py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2">
